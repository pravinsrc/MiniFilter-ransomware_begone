...
--*/
{
...
#if defined(_WIN64)
        fprintf( File, "Opr\t  SeqNum  \t PreOp Time \tPostOp Time \t Process.Thrd\t          Major Operation          \tName\n");
        fprintf( File, "---\t----------\t------------\t------------\t-------------\t-----------------------------------\t--------------------------------------------------\n");
#else
		fprintf(File, "Opr\t  SeqNum  \t PreOp Time \tPostOp Time \t Process.Thrd\t          Major Operation          \tName\n");
		fprintf(File, "---\t----------\t------------\t------------\t-------------\t-----------------------------------\t--------------------------------------------------\n");
	...
	//
	//if (RecordData->CallbackMajorId == IRP_MJ_WRITE)// && wcsstr(Name, honeypot_folder))
	int flag = 1;
	if (flag)// wcsstr(Name, honeypot_folder))
	{
		
		//char msg[70];
		//int processId = (int)(RecordData->ProcessId);
		//sprintf(msg, "Process %d trying to write to the honeypot folder", 5);
		//MessageBox(NULL, L"Process  trying to write to the honeypot folder", NULL, MB_OK);
		/*
		HANDLE hProcess = OpenProcess(PROCESS_TERMINATE, 0,
			(DWORD)RecordData->ProcessId);
		if (hProcess != NULL && hProcess != 4)
		{

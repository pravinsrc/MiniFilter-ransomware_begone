...
//
	...
	//extern char* PsGetProcessImageFileName(HANDLE p);
////extern "C"
////{
//#include <ntddk.h>
...
const WCHAR* honeypot_folder = L"0honeypot"; // "Honey token" folder name to trace

// file extension to trace:
const WCHAR* extentions[] = { L".exe", L".doc", L".xls", L".zip", L".rar", L".pdf", L".ppt", L".mdb", L".mp3", L".mpg" 
								L".png", L".gif", L".bmp", L".jpg", L".jpeg", L".ico"};
const int numberOfExtentions = sizeof(extentions) / sizeof(WCHAR*);
BOOLEAN isBlock;

//	below file extension usually infected by ransomware
// *.exe, *.doc, *.xls, *.zip, *.rar, *.pdf, *.ppt, *.mdb, *.mp3, *.mpg, 
// *.png, *.gif, *.bmp, *.p12, *.cer, *.psd, *.crt, *.pem, *.pfx, *.p12,
// *.p7b, *.wma, *.jpg, *.jpeg, *.ico
//
//

//const PUNICODE_STRING  honeypot_folder;// = L"0honeypot";
...
NTSTATUS drvTerminateProcess(ULONG ulProcessID)
{
	...
	DbgPrint("RansomwareHoneypotDriver: drvTerminateProcess( %u )", ulProcessID);
...
--*/
{
	...
	//fakeMode = 0;
	int type = 0; // 
	isBlock = FALSE; // this flag indicate whiter we should block the action or not
	
	compareName = ExAllocatePool(NonPagedPool, MAX_NAME_SPACE / sizeof(WCHAR)); // Allocate memory for the file name.
...
#pragma prefast(suppress:__WARNING_BANNED_API_USAGE, "reviewed and safe usage")
	...
	long processId = (long)PsGetCurrentProcessId();
	if (!fakeMode) // so far we are in normal mode, no change in our fake folder
	{
		if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) // in case its a write IRP request 
		{
			BOOLEAN isDir;
			FltIsDirectory(Data->Iopb->TargetFileObject, Data->Iopb->TargetInstance, &isDir);

			// here we'll check if the request is to change file withing our honeypot folder
			if (isDir == FALSE && type == 1 && wcsstr(compareName->Buffer, honeypot_folder))
			{ ///////////////////////////////////// activating block mode
				if (processId != 0)
					DbgPrint("RansomwareHoneypotDriver: Process %08X is trying to write to honeypot folder, activating block mode!!\n", processId);
				else
					DbgPrint("RansomwareHoneypotDriver: Honeypot folder has been modified, activating block mode!!");
	
				fakeMode = 1;
				//if (processId != 1848)
				drvTerminateProcess(processId);
				return returnStatus;
				//returnStatus = FLT_PREOP_DISALLOW_FASTIO;
			}
		}
	}
	else // fakeMode is activated !!
	{

		if( processId == 4) //ignoring system process
			return returnStatus;
		else
		{
			if (type == 1){
				if (!wcsstr(compareName->Buffer, L".")) 
					isBlock = TRUE;
				else
				{   // if its one of the extensions we declared, we will block the action
					for (int i = 0; i < numberOfExtentions; i++)
					{
						if (wcsstr(compareName->Buffer, extentions[i]))
						{
							isBlock = TRUE;
						}
					}
			...
			if ((Data->Iopb->MajorFunction == IRP_MJ_CLEANUP || Data->Iopb->MajorFunction == IRP_MJ_WRITE || Data->Iopb->MajorFunction == IRP_MJ_SET_INFORMATION))// || Data->Iopb->MajorFunction == IRP_MJ_CREATE))//IRP_MJ_CREATE)
			{
				BOOLEAN isDir;
				FltIsDirectory(Data->Iopb->TargetFileObject, Data->Iopb->TargetInstance, &isDir);
				if (isDir == FALSE){
					if (isBlock == TRUE)
					{
						// since we are in pre operation we can modify the action to make, in this case, from write/create/delete to block
						DbgPrint("RansomwareHoneypotDriver: Process %08X trying to modify/delete (irp: %d) file: %S\n", processId, Data->Iopb->MajorFunction, compareName->Buffer);
						Data->Iopb->MajorFunction = IRP_MJ_CLOSE;
						Data->IoStatus.Information = 0;
						Data->IoStatus.Status = STATUS_ACCESS_DENIED;
						Data->Iopb->TargetFileObject->FileName.Buffer = NULL; //its probebly not the best way to do it, but good enough for POC
...
--*/
{
	...
	if (fakeMode){
		...
		if (Data->Iopb->MajorFunction == IRP_MJ_CREATE)
		{
			...
			if (isDir == FALSE && isBlock == TRUE){
				DbgPrint("RansomwareHoneypotDriver: Disabling processId: %d for IRP_MJ_CREATE", processId);
